    ...
    public partial class Front : Form
    {
        ...
        public void updatePictureBox()
        {
            ...
            foreach (var Image_Items in ImageLayer.Controls.OfType<PictureBox>())
            {
                ...
                {
                    if (Session.productPos[pic.Name].link_image == "" || Session.productPos[pic.Name].link_image == null)
                    {
                        Session.productPos[pic.Name].link_image = "noimage.png";
                    }
                    if (Session.productPos[pic.Name].link_image == "" && Session.productPos[pic.Name].RFIDcode != "")
                    {
                        pic.Load("noimage.png");
                    }
                    else if (Session.productPos[pic.Name].link_image == "" && Session.productPos[pic.Name].RFIDcode == "")
                    {
                        pic.Load("blank_background.png");
                    }
                    else if (Session.productPos[pic.Name].RFIDcode == "")
                    {
                        pic.Load("blank_background.png");
                    }
                    else
                    {
                        pic.Load(Session.productPos[pic.Name].link_image);
                    }
            ...
            private void Pic_Paint(object sender, PaintEventArgs e)
        {
            ...
            try
            {
                using (Font myFont = new Font("Arial", 10))
                {
                    ...
                    string s = Session.productPos[Session.key_drawtext].product_name;
                    ...
                    centered.Offset(0, (int)(e.ClipRectangle.Height - e.Graphics.MeasureString(s, this.Font).Height));

                    e.Graphics.DrawString(s, myFont, br, centered, sf);

                    Console.WriteLine($"CALL PAINT EVENT {((PictureBox)sender).Name}");

                    //e.Graphics.DrawString(Session.productPos[Session.key_drawtext].product_name, myFont, Brushes.Green, centered, sf); 
                }
            }

            catch
            {
                Console.WriteLine("Out Paint");
            }
        }
                ...
                foreach (PictureBox pic in ImageLayer.Controls.OfType<PictureBox>())
                {
                    if (Session.productPos.Keys.Contains(pic.Name))
                    {
                        ...
                        json = System.Text.Json.JsonSerializer.Serialize(new
                        {
                            ...
                            dpp_image_url = Session.productPos[pic.Name].link_image
                        }
                        );
                    }
                    else
        ...
        private async Task ApiGetSmartShelfStatus(string name, string rfid)
        {
            ...
            {
                Console.WriteLine("Failed to get Smart Self status \n");
            }
        ...
        private async Task ApiGetSmartShelfNames()
        {
            ...
            {
                Console.WriteLine("Failed to  get Smart Self names \n");
            }
        ...
        private void pictureBox_Click(object sender, EventArgs e)
        {
            ...
            if (lastChoose == null)
            {
                if (choosingImage.ImageLocation == "blank_background.png")
                {
                    ...
                    if (Session.product.link_image != "" && !Session.productPos.Keys.Contains("temp"))
                    {
            ...
            else if (choosingImage.ImageLocation == "blank_background.png")
            {
                ...
                if (Session.productPos.ContainsKey(lastChoose.Name))
                {

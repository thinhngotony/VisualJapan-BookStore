...
namespace SelfRegi_V2
{
    public partial class Front : Form
    {
        ...
        public Front()
        {
            ...
            //if (Session.reload.Equals("0"))
            //{
            //    btnDataReq.Visible = false;
            //}
            //else
            //{
            //    btnDataReq.Visible = true;
            //}
        ...
        private void init()
        {
            ...
            //List<string> listKey = new List<string>(new string[] { "api_key", "url_api", "sub_url", "path", "device_name" });
            ...
            //dt.DefaultView.Sort = "gds_jan_cd asc";
        ...
        public void updateView()
        {
            ...
            lJancode2.Text = Session.product.Jancode2;
        ...
        private async Task Api_Recv2()
        {
            try
            {
                ...
                if (result.IsSuccessStatusCode)
                {
                    ...
                    if (api_status.Equals("00"))
                    {
                        Session.product.ccode = (string)data["data"]["drgm_c_code"] == null ? "//" : (string)data["data"]["drgm_c_code"];
                        Session.product.artist_name = (string)data["data"]["drgm_artist"] == "" ? "//" : (string)data["data"]["drgm_artist"];
                        Session.product.Jancode = Session.barcode;
                        Session.product.Jancode2 = (string)data["data"]["drgm_jan2"];
                        Session.product.makerCD = (string)data["data"]["drgm_maker_cd"] == "" ? "//" : (string)data["data"]["drgm_maker_cd"];
                        Session.product.goods_type = (string)data["data"]["rf_goods_type"];
                        Session.product.goods_name = (string)data["data"]["drgm_goods_name"];
                        Session.product.media_cd = (string)data["data"]["drgm_media_cd"];
                        Session.product.price = (int)data["data"]["bqsq_shop_goods_price"];
                        Session.product.tax_rate = 1.1;
                        //Session.product.price = Convert.ToInt32(Math.Floor(Session.product.price * Session.product.tax_rate)); 
                        Session.product.price = (int)data["data"]["bqsq_shop_goods_price"];
        ...
        private int isDataValid()
        {
            ...
            if (txtArtist.Text.Equals("") && txtArtist.ReadOnly == false) return 2;
            if (txtPName.Text.Equals("") && txtPName.ReadOnly == false) return 3;
            if (txtPrice.Text.Equals("") && txtPrice.ReadOnly == false) return 4;
            return 0;
        }
        ...
        private void btnRegister_Click(object sender, EventArgs e)
        {
            ...
            if (isDataValid() == 0)
            {
                ...
                //richTextBox1.Text += DateTime.Now.ToString("hh:mm:ss: Loading ...\n");
                ...
                //using (Wait wait = new Wait(async () => await Task.Run(() => ApiSend())))
                //{
                //    richTextBox1.Text += DateTime.Now.ToString("hh:mm:ss") + ": Loading ...\n";
        ...
        private void btnDelete_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                DialogResult confirmResult = MessageBox.Show("削除モードに切り替えてもよろしいですか？", "Confirm Diaglog", MessageBoxButtons.YesNo);
                if (confirmResult == DialogResult.No)
                {
